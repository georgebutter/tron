        ...
        db.collection('games')
          ...
          .onSnapshot((snap) => {
            if (!snap.empty) {
              ...
              this.statusText
                .setText(`You are player ${this.player.player}`)
                .setStyle({
                  fill: this.player.colour.string,
                  fontSize: '25px',
                  shadow: {
  ...
  public update() {
    ...
    if (this.pause) {
      ...
      for (const l of this.lines) {
        if (l.keys && (JustDown(l.keys.left) || JustDown(l.keys.right))) {
          l.ready = true;
          l.alive = true;
          l.tail = [];
        ...
        if (l.ready) {
          l.readyText.setText(`Player ${l.player} - Ready`);
          readyLines.push(l);
        } else {
          l.readyText.setText(`Player ${l.player} - Not ready`);
        }
      }
    ...
    for (const l of this.lines) {
      if (l.alive) {
        if (l.keys && JustDown(l.keys.left)) {
          this.turnLeft(l);
        } else if (l.keys && JustDown(l.keys.right)) {
          this.turnRight(l);
        }
        this.growRect(l);
...
export interface OnlineGameScene {
  pause: boolean;
  allReady: boolean;
  lines: PlayerLine[];
  allLines: PlayerLine[];
